...
class SDSeg(LatentDiffusion):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=True):
        ...
        if not only_model:
            ...
            # 优化的参数匹配和加载逻辑
            print(f"\033[33m[INFO]: 开始匹配和加载预训练参数...\033[0m")
            ...
            pretrain_keys = set(pretrain_sd.keys())

            # 遍历预训练参数进行匹配
            for pk in pretrain_keys:
                if pk in current_keys:
                    pshape, shape = pretrain_sd[pk].shape, sd[pk].shape

                    # 形状完全匹配
                    if pshape == shape:
                        compatible_sd[pk] = pretrain_sd[pk]
                        matched_count += 1
                        print(f"\033[32m[MATCH]: {pk} - {shape}\033[0m")

                    # 形状不匹配但维度相同，尝试调整
                    elif len(pshape) == len(shape):
                        if len(pshape) == 4:  # 卷积层权重
                            # 如果只是通道数不同，可以尝试填充或截断
                            if pshape[0] == shape[0] and pshape[2:] == shape[2:]:  # 输出通道和空间维度相同
                                if pshape[1] < shape[1]:  # 输入通道少，需要填充
                                    padding_size = shape[1] - pshape[1]
                                    padding = torch.zeros(pshape[0], padding_size, *pshape[2:])
                                    compatible_sd[pk] = torch.cat([pretrain_sd[pk], padding], dim=1)
                                    print(f"\033[31m[PADDING]: {pk} from {pshape} to {shape}\033[0m")
                                    matched_count += 1
                                elif pshape[1] > shape[1]:  # 输入通道多，需要截断
                                    compatible_sd[pk] = pretrain_sd[pk][:, :shape[1], :, :]
                                    print(f"\033[31m[TRUNCATE]: {pk} from {pshape} to {shape}\033[0m")
                                    matched_count += 1
                                else:
                                    print(f"\033[31m[SHAPE_MISMATCH]: {pk} - {pshape} vs {shape}\033[0m")
                                    shape_mismatch_count += 1
                            else:
                                print(f"\033[31m[SHAPE_MISMATCH]: {pk} - {pshape} vs {shape}\033[0m")
                                shape_mismatch_count += 1
                        elif len(pshape) == 1:  # 偏置项
                            if pshape[0] == shape[0]:
                                compatible_sd[pk] = pretrain_sd[pk]
                                matched_count += 1
                                print(f"\033[32m[MATCH]: {pk} - {shape}\033[0m")
                            elif pshape[0] < shape[0]:  # 需要填充
                                padding = torch.zeros(shape[0] - pshape[0])
                                compatible_sd[pk] = torch.cat([pretrain_sd[pk], padding])
                                print(f"\033[31m[PADDING]: {pk} from {pshape} to {shape}\033[0m")
                                matched_count += 1
                            else:  # 需要截断
                                compatible_sd[pk] = pretrain_sd[pk][:shape[0]]
                                print(f"\033[31m[TRUNCATE]: {pk} from {pshape} to {shape}\033[0m")
                                matched_count += 1
                        else:
                            print(f"\033[31m[SHAPE_MISMATCH]: {pk} - {pshape} vs {shape}\033[0m")
                        ...
                        print(f"\033[31m[DIM_MISMATCH]: {pk} - {len(pshape)}D vs {len(shape)}D\033[0m")
                        shape_mismatch_count += 1
                else:
                    print(f"\033[31m[KEY_MISMATCH]: {pk} not found in current model\033[0m")
                    key_mismatch_count += 1

            # 将兼容的参数复制到当前模型的状态字典
            for k, v in compatible_sd.items():
                sd[k] = v
            ...
            # 输出统计信息
            print(f"\033[32m[SUMMARY]: 成功匹配 {matched_count} 个参数\033[0m")
            print(f"\033[31m[SUMMARY]: 形状不匹配 {shape_mismatch_count} 个参数\033[0m")
            print(f"\033[31m[SUMMARY]: 键名不匹配 {key_mismatch_count} 个参数\033[0m")
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        ...
        c_concat = [item[0] if isinstance(item, tuple) else item for item in c_concat]
